...
- `editable` — Keep the resulting PDF fillable or flatten the form fields. See [Editable PDFs](#editable-pdfs).
...
- **Per submission:** Set `editable: true` in your submission request to make every field editable, or `editable: false` to flatten every field.
- **Per field:** Enable the "Editable" option on an individual field in the Template Editor. That field is always rendered as an editable form field, even when the submission's `editable` option is unset or set to `false`.

If you leave the submission `editable` option unset, each field falls back to its own "Editable" setting.
