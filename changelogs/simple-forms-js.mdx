...
### 2.6.1 <span class="ml-2.5 relative -top-0.5"><Badge text="Latest" variant="success" size="small"></Badge></span>

<div class="mt-2.5 text-sm text-gray-500 dark:text-gray-400">
  Released on Aug 18, 2026
</div>

- Updated the [conditional logic](/docs/template-editor/conditional-logic/) support
  for the new standards-compliant JSON schema output, where conditionally-shown
  fields are defined inside the schema's `if`/`then` conditionals. Templates using
  conditional logic must use this version (or later) of the library.
