...
### 2.3.7 <span class="ml-2.5 relative -top-0.5"><Badge text="Latest" variant="success" size="small"></Badge></span>
...
- Fixed preconfigured `DocSpring.appHost`, `DocSpring.host`, `DocSpring.scheme`,
  and `DocSpring.port` values being overwritten when the library loads.
...
- Fixed the signature modal drifting off-screen when pinch-zooming on mobile devices.
  The parent page now notifies the embedded signing form whenever its visual viewport
  changes (e.g. pinch-to-zoom or pan), so the signature modal can reposition itself
  and stay in view.
