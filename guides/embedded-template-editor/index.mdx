...
<Aside type="note">
  Enterprise deployments can load this script from the local DocSpring domain
  instead of the CDN:
  `https://docspring.yourcompany.com/assets/embed/template_editor.js`.
</Aside>
...
| `region`                                     | string  | No       | Use a DocSpring region: `US` or `EU`. Defaults to `US`.                                                         |
...
Use `region` when your account is in the EU region:
...
<Code
  ...
  lang="javascript"
  title="Regional embedded editor"
/>
...
For split-domain deployments, use `appHost` and `apiHost`. Existing globals such
