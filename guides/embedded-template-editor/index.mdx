...
<Aside type="note">
  Modal editors close automatically after `onDone` and `onEvent` run. Use the
  returned `close()` function (or `DocSpring.closeTemplateEditor()`) when you
  need to close a modal from another part of your application.
</Aside>
